...
import { useAppContext } from '@/contetx/AppContext'
  ...
  return (
    <div className={`flex flex-col justify-between bg-[#212327] pt-7 transition-all z-50 max-md:absolute max-md:h-screen ${expand ? 'p-4 w-64' :'md:w-20 w-0 max-md:overflow-hidden '}`}>
      <div>
        ...
        <button className={`mt-8 flex items-center justify-center cursor-pointer ${expand ? 'bg-primary opacity-90 rounded-2xl gap-2 p-2.5 w-max ' :'group relative h-9 w-9 hover:bg-gray-500/30 rounded-lg'}`}>
            <Image className={expand ? 'w-5' :'w-7'} src={expand ? assets.chat_icon : assets.chat_icon_dull} alt='' ></Image>
        ...
        <div>
            <div className={`flex items-center cursor-pointer group relative ${expand ?'gap-1 text-white/80 text-sm border border-primary rounded-lg hover:bg-white/10 cursor-pointer ': 'h-10 w-10 mx-auto hover: bg-gray-500/30 rounded-lg'}`}>
                ...
                <div className='relative w-max bg-black text-white text-sm p-3 rounded-lg shadow-lg'>
                    <Image src={assets.qrcode} alt='' className='w-44'></Image>
                    <p>Scan to get DeepSeek App</p>
                    <div className={`w-3 h-3 absolute bg-black rotate-45  ${expand ? 'right-1/2 ' : 'left-4'} -bottom-1.5 `}></div>
                </div>
            </div>
            {expand && <><span>Get App</span> <Image alt='' src={assets.new_icon} className='w-6 h-5 border'></Image></>}
            </div>
